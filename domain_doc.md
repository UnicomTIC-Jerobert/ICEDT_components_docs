...
---
** " i heard that there is another straight forward method to connected ec2 with the domain i purchased from cloudflare
since you know my requirements and you have a record that what things we have up to now , so shall we create A subdomain A record such as "api-icedt.test123.work" and connect to ec2 staging server , mean time we have to handle the HTTPS issue as well , and i think we dont need to use "Route 53 " in this screnario , and explain me the nginx setup needed and you have memory of our nginx configuration as well" **
